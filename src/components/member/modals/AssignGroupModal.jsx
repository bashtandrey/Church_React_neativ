...
export default function AssignGroupModal({
  ...
  useEffect(() => {
    if (visible) {
      setSelected(member?.groupId || null);
      loadData();
    }
  }, [visible]);
  ...
  const loadData = async () => {
    ...
    setErr("");
    try {
      const list = await fetchAllMemberGroup();
      const mapped = list.map((g) => ({
        id: g.id,
        name: g.name,
        leader: g.leader ? `${g.leader.firstName} ${g.leader.lastName}` : null,
      }));
      setOptions([{ id: null, name: "— Нет —", leader: null }, ...mapped]);
    } catch (e) {
      setErr(e?.message || "Ошибка загрузки групп");
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <View style={styles.centerCard}>
          ...
          <View style={styles.header}>
            <Text style={styles.title}>Назначить группу</Text>
              ...
              options.map((opt) => {
                ...
                return (
                  ...
                  >
                    ...
                    {opt.leader && (
                      <Text style={styles.optionLeader}>
                        Лидер: {opt.leader}
                      </Text>
                    )}
                  </Pressable>
